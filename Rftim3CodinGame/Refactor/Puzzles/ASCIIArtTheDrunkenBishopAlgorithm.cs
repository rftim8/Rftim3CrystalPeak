...
{
    public class ASCIIArtTheDrunkenBishopAlgorithm
    {
        public ASCIIArtTheDrunkenBishopAlgorithm()
        {
            string input = "fc:94:b0:c1:e5:b0:98:7c:58:43:99:76:97:ee:9f:b7";
        ...
        /// For example, if the bishop is on the right edge of the board and the next move is 01, we simply slide the bishop one space vertically up, 
        /// incrementing the counter on the new square as normal.
        /// If the Bishop ends up at a corner and he cannot slide, he simply does not move(and we should increment counter for that corner cell). 
        /// Here the four cases:
        ...
        /// 
        /// At the end of the 64 steps, the board is drawn assigning a symbol to each position on the board according to how many times it was visited.
        /// OpenSSH uses these symbols:
        /// 
        ...
        public static List<string> RftSolve(string input)
        {
            List<string> h = [.. input.Split(':')];

            char[][] map = new char[19][];

            for (int i = 0; i < 11; i++)
            {
                if (i == 0) map[i] = "+---[CODINGAME]---+".ToCharArray();
                else if (i == 10) map[i] = "+-----------------+".ToCharArray();
                else map[i] = "|                 |".ToCharArray();
            }

            int[,] vis = new int[11, 19];
            ...
            {
                string a = Convert.ToString(Convert.ToInt32(item[0].ToString(), 16), 2).PadLeft(4, '0');
                string b = Convert.ToString(Convert.ToInt32(item[1].ToString(), 16), 2).PadLeft(4, '0');

                List<string> steps = [b[2..], b[..2], a[2..], a[..2]];

                foreach (string item1 in steps)
                {
                    vis[x, y]++;
            ...
            for (int i = 0; i < 11; i++)
            {
                for (int j = 0; j < 19; j++)
                {
                    if (vis[i, j] > 14) vis[i, j] = vis[i, j] % 14 - 1;
                }
            }
            ...
            for (int i = 1; i < 10; i++)
            {
                ...
                {
                    if (i == x && j == y) t += "E";
                    else if (i == origX && j == origY) t += "S";
                    else
                    {
                        if (vis[i, j] != 0) t += $"{s[vis[i, j] - 1]}";
                        else t += " ";
                    }
                }
                t += '|';
