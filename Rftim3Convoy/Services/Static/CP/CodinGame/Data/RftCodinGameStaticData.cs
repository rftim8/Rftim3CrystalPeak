namespace Rftim3Convoy.Services.Static.CP.CodinGame.Data
{
    public class RftCodinGameStaticData : IRftCodinGameStaticData<List<string>, List<string>>
    {
        ...
        private static List<string> GetFileContentToList(bool testType, bool direction, string problemName)
        {
            ...
            path = direction ?
                $"{path[..indexes[^subfolderLevel]]}\\Rftim3Atlas\\CP\\CodinGame\\IO\\{problemName}_Input.txt" :
                $"{path[..indexes[^subfolderLevel]]}\\Rftim3Atlas\\CP\\CodinGame\\IO\\{problemName}_Output.txt";
            List<string> list = [];
